...
export class SocketAdapterImpl implements SocketAdapter {
  ...
  private _onConnectCallback: (<T extends this>(socketAdapter: SocketAdapter, data: T) => void) | null;
  ...
  onConnect<T>(callback: (socketAdapter: SocketAdapter, data: T) => void): void {
    ...
    this._onConnectCallback = callback as <T>(socketAdapter: SocketAdapter, data: T) => void
  }

  onDisConnect<T>(callback: (socketAdapter: SocketAdapter, data: T) => void): void {
    this._onDisConnectCallback= callback as <T>(socketAdapter: SocketAdapter, data: T) => void
  }

  onError<T>(callback: (socketAdapter: SocketAdapter, data: T) => void): void {
    this._onErrorCallback = callback as <T>(socketAdapter: SocketAdapter, data: T) => void
  }
